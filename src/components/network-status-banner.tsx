      ...
      <div className="relative flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="relative flex h-2.5 w-2.5">
            <span className="absolute inline-flex h-full w-full animate-ping rounded-full opacity-75 bg-blue-600"></span>
        ...
        <div className="flex items-center gap-4">
          <p className="hidden font-medium text-zinc-900 dark:text-zinc-50 text-sm tracking-wide md:inline">
            Contract: <span className="font-mono text-zinc-600 dark:text-zinc-400 font-normal">{appConfig.contractAddress ? `${appConfig.contractAddress.slice(0, 6)}...${appConfig.contractAddress.slice(-4)}` : "Demo Mode"}</span>
          </p>
          <span className={`rounded-xl px-3 py-1 font-medium text-xs tracking-wide ${appConfig.isLive ? 'bg-blue-600/15 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' : 'bg-amber-500/10 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400'}`}>
            {appConfig.isLive ? "LIVE" : "DEMO"}
          </span>
        </div>
      </div>
    </div>
