...
class PredictionMarket(gl.Contract):
    # ── Storage ──────────────────────────────────────────────────────
    next_market_id: u256
    markets: TreeMap[u256, str]               # market_id -> JSON blob (market data)
    bets_yes: TreeMap[u256, TreeMap[str, u256]] # market_id -> user_address -> amount
    bets_no: TreeMap[u256, TreeMap[str, u256]]  # market_id -> user_address -> amount
    pool_yes: TreeMap[u256, u256]             # market_id -> total YES pool
    pool_no: TreeMap[u256, u256]              # market_id -> total NO pool
    claimed: TreeMap[u256, TreeMap[str, bool]] # market_id -> user_address -> bool
    ...
    def bet(self, market_id: int, is_yes: bool) -> None:
        mid_key = u256(market_id)
        market = self._load_market(mid_key)
        ...
        if is_yes:
            if mid_key not in self.bets_yes:
                self.bets_yes[mid_key] = TreeMap()
            if sender not in self.bets_yes[mid_key]:
                self.bets_yes[mid_key][sender] = u256(0)
            self.bets_yes[mid_key][sender] += amount
            
            current_pool = self.pool_yes.get(mid_key, u256(0))
            self.pool_yes[mid_key] = current_pool + amount
        else:
            if mid_key not in self.bets_no:
                self.bets_no[mid_key] = TreeMap()
            if sender not in self.bets_no[mid_key]:
                self.bets_no[mid_key][sender] = u256(0)
            self.bets_no[mid_key][sender] += amount
            
            current_pool = self.pool_no.get(mid_key, u256(0))
            self.pool_no[mid_key] = current_pool + amount
...
{{"outcome": "yes" or "no" or "invalid", "reasoning": "brief explanation based on evidence"}}"""
        ...
        self._save_market(mid_key, market)
    ...
    def claim_winnings(self, market_id: int) -> None:
        ...
        if mid_key not in self.claimed:
            self.claimed[mid_key] = TreeMap()
        if self.claimed[mid_key].get(sender, False):
            raise gl.vm.UserError("Winnings already claimed")
            
        pool_yes = self.pool_yes.get(mid_key, u256(0))
        pool_no = self.pool_no.get(mid_key, u256(0))
        ...
        if outcome == "yes":
            bet_yes = u256(0)
            if mid_key in self.bets_yes and sender in self.bets_yes[mid_key]:
                bet_yes = self.bets_yes[mid_key][sender]
            ...
            bet_no = u256(0)
            if mid_key in self.bets_no and sender in self.bets_no[mid_key]:
                bet_no = self.bets_no[mid_key][sender]
        ...
        self.claimed[mid_key][sender] = True
    ...
    def get_market(self, market_id: int) -> str:
        mid_key = u256(market_id)
        raw = self.markets.get(mid_key, "")
        if not raw:
            return json.dumps(None)
            
        data = json.loads(raw)
        data["pool_yes"] = str(self.pool_yes.get(mid_key, u256(0)))
        data["pool_no"] = str(self.pool_no.get(mid_key, u256(0)))
        return json.dumps(data)
        
    @gl.public.view
    def list_markets(self) -> str:
        result = []
        for mid_key in self.markets:
            raw = self.markets[mid_key]
            if raw:
                doc = json.loads(raw)
                doc["pool_yes"] = str(self.pool_yes.get(mid_key, u256(0)))
                doc["pool_no"] = str(self.pool_no.get(mid_key, u256(0)))
                result.append(doc)
    ...
    def get_bet(self, market_id: int, user_address: str) -> str:
        ...
        if mid_key in self.bets_yes and user_address in self.bets_yes[mid_key]:
            bet_yes = self.bets_yes[mid_key][user_address]
        if mid_key in self.bets_no and user_address in self.bets_no[mid_key]:
            bet_no = self.bets_no[mid_key][user_address]
            
        claimed = False
        if mid_key in self.claimed and user_address in self.claimed[mid_key]:
            claimed = self.claimed[mid_key][user_address]
